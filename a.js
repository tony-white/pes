...
var checkSettingsElement = document.querySelector('script[type="application/json"][data-drupal-selector="drupal-settings-json"]') ? document.querySelector('script[type="application/json"][data-drupal-selector="drupal-settings-json"]') :  false;
if (checkSettingsElement) {
  var checkDrupalSettings= JSON.parse(checkSettingsElement.textContent);
}

// Modification to only load if user is Logged out to stop conflicting
// issues with the inline editing tools.
...
function setStorage(name, data, expiry, useCookie) {
    ...
    /// <param name="expiry" type="number">Number of days and hours before expiry e.g. 1.12 for 1 day 12 hours</param>
...
// If search results page add the dynmaic breadcrumb
var setSearchBreadbrumb = (function() {
    var storage = getStorage('search_page', true);

    if(storage) {
        var value = JSON.parse(storage),
            html = '<div class="column is-align-center ie-is-8"><a href="{{URL}}" class="search-breadcrumb" title="Return to {{TITLE}}">Return to {{TITLE}}</a></div>',
...
var searchFilterStyling = (function() {
    var form = $('#search-page-form'),
        ...
        type = $('#block-contentorfiletype').clone(),
        taxonomy = $('#block-servicetaxonomy').clone(),
        search = $('.js-form-item-aggregated-field').clone(),
        sort = $('.js-form-item-sort-by').clone(),
        count = $('.count-wrapper');


    // Remove and empty elements
    form.empty();
    $('#block-contentorfiletype').remove();
    $('#block-servicetaxonomy').remove();

    // Append new core markup
    form.append('<div class="search-container"></div>');
    form.append('<div class="filter-container"></div>');
    form.append('<div class="support-container"></div>');

    // Append elements
    $('.search-container').append(search);
    $('.search-container').append(actions);

    $('.filter-container').prepend('<h3>Filter results by:</h3>')
    $('.filter-container').append('<div class="taxonomy-wrapper"></div><div class="type-wrapper"></div>');
    $('.taxonomy-wrapper').append(taxonomy);
    $('.type-wrapper').append(type);

    $('.support-container').append('<div class="sort-wrapper"></div>');
    $('.support-container').prepend(count);
    $('.sort-wrapper').append(sort);



    $('#edit-sort-by').on('change', function () {
        var string = '&sort_by=' + $(this).val(),
            fullURL = window.location.href,
            param = getParam('sort_by');

        if (param) {
            var finalURL = fullURL.replace(param, $(this).val());
        } else {
            var finalURL = fullURL + string;
        }

        window.location.href = finalURL;
    });

    // Convert UL's into Selects
    if (isLoggedIn === false) {
        // Code taken from core DRUPAL8
        // /nhsbsa/project/modules/contrib/facets/js/dropdown-widget.js
        $('.js-facets-dropdown-links').each(function () {
...
function mobileNavigation() {
    ...
    var menuString = $('.js-mobile-menu-container').html(), // get html menu string
        menuFrag = convertToFrag(menuString), // Create initial menu frag
    ...
    var menuWithChild = menu.find('.menu-item--expanded'),
        toggleLevel = '<button class="js-toggle-level level-toggle"><span class="sr-only">Toggle level</span></button>',
        closeMenu = '<button class="js-close-menu close-menu"><span class="sr-only">Close menu</span></button>';

    for (var i = 0; i < menuWithChild.length; i++) {
        if ($(menuWithChild[i]).parent('.menu').hasClass('depth-0')) {
            var thisIcon = convertToFrag(closeMenu);
            ...
            var thisIcon = convertToFrag(toggleLevel);
    ...
    $('.js-mobile-menu-container').remove();
    body.prepend(menu);
    var home = '<li class="menu-item menu-item--home"><a href="/" class="link-level-0">Return to NHSBSA home</a></li>';
...
var shareThisPage = (function() {
    var container = $('.js-shareThisPage'),
        ...
        button = '<button class="js-shareThisPageButton share-this-page-button">Share this page</button>';
...
$(document).ready(function () {
    ...
    jumplinks.each(function (key, val) {
        ...
        var strippedText = document.createTextNode($(val).html());
        container.appendChild(strippedText);

        $(li).html('<a href="#jumplink'+key+'">' + container.innerHTML + '</a>');

        linkList.appendChild(li);

        if (wrapper.length > 0) {
            $(this).attr('id', 'jumplink' + key).append('<a href="#jumplinktop" class="back-to-top"></a>');
        } else {
            $(this).attr('id', 'jumplink' + key).append('<a href="#top" class="back-to-top"></a>');
        }
    });
...
var newsBlocks = (function () {
    ...
    for (var i = 0; i < arrayRows.length; i++) { // Loop through rows
        var thisRow = arrayRows[i],
          ...
          html = "", // HTML string
          htmlOpen = "<div class='news-block-container column is-block'>", // Opening HTML
          htmlClose = "</div></div>"; // Closing HTMl
...
var contextualToggle = (function () {
    var container = $('.contextual-tabs .contextual-region'),
      html = '<li class="contextual-toggle-wrapper"><button class="js-contextual-toggle contextual-toggle">Options</button></li>';
...
var responsive_tables = (function () {
    ...
    for (var i = 0; i < tables.length; i++) {
        var this_table = $(tables[i]),
          html = $('<div class="responsive_table"></div>');
...
var newsListingFilter = function () {
    ...
    html += '<div class="news-listing-years"><p><strong>Year:</strong><p>';
    html += '<div><select class="year-select">';
    html += '<option>Filter by year</option>';

    for (var i = 0; i < years.length; i++) {
        if (years[i] === param) {
            html += '<option selected>' + years[i] + '</option>';
        } else {
...
if ($('.year-dropdown-content').html()) {
    var newsListingYears = JSON.parse($('.year-dropdown-content').html());
    if (typeof(newsListingYears) != "undefined") {
        newsListingFilter();
    }
}

// set admin toolbar breakpoint
if (typeof drupalSettings !== "undefined") {
    if(drupalSettings.toolbar !== undefined){
        drupalSettings.toolbar.breakpoints['toolbar.wide'] = 'only screen and (min-width: 74.375em)';
    }
}

// Datepicker jQuery UI - IE9 jump fix
$(document).on('click', '.hasDatepicker', function(){
    $('#ui-datepicker-div a').attr('href', 'javascript: void(0)');
});